...
public class Rigid_Bunny : MonoBehaviour
{
    bool launched = false;
    float dt = 0.015f;
    Vector3 v = new Vector3(0, 0, 0);   // 速度
    Vector3 w = new Vector3(0, 0, 0);   // 角速度

    float mass;                                 // 质量
    Matrix4x4 I_ref;                            // 参考惯性

    float linear_decay = 0.999f;                // 用于速度衰减
    float angular_decay = 0.98f;
    float restitution = 0.5f;                 // 用于碰撞
    float friction = 0.2f;                  
    ...
    void Collision_Impulse(Vector3 P, Vector3 N)
    {
        ...
        Vector3 r_collision = sum / (float)collisionNum;                // 虚拟碰撞点（局部协调）
        Vector3 Rr_collision = R.MultiplyVector(r_collision);
        //Vector3 x_collision = T + Rr_collision;							 // 虚拟碰撞点（全局协调）
